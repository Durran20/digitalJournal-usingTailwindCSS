<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] = "POST") {
        ...
        $query = mysqli_query($con, "SELECT * from journal.personalentry WHERE username='$username'");  // Retrieves all data belonging to the specified username

        // Check if user exists
        $exists = mysqli_num_rows($query);  // If it exits, the number that $exists will hold will be greater than 0

        mysqli_query($con, "INSERT INTO `journal`.`personalentry` (`username`, `entry`, `date`) VALUES ('$user', '$personalEntry', '$date');");
